...
KO_TAGS ?= $(shell git describe --tags --always --dirty)
KO_DOCKER_REPO ?= 590183687488.dkr.ecr.us-west-2.amazonaws.com/karpenter/aws
KOCACHE ?= ~/.ko
...
image: ## Build the Karpenter controller images using ko build
	$(eval CONTROLLER_IMG=$(shell $(WITH_GOFLAGS) KOCACHE=$(KOCACHE) KO_DOCKER_REPO="$(KO_DOCKER_REPO)" ko build --tags $(KO_TAGS) --bare github.com/aws/karpenter-provider-aws/cmd/controller))
